...
- **preload**: `src/preload/index.ts` — exposes `window.api` with namespaces `fetchImage`, `fetchSvg`, `app`, `settings`, `updater`, `windowControls`, `lan`, `ssh`, `sticky`, `k8s`. Ambient API types are split per namespace across `src/preload/*.d.ts` (`lan.d.ts`, `ssh.d.ts`, `k8s.d.ts`, `sticky.d.ts`, `updater.d.ts`) and composed into `Api`/`Window.api` in `index.d.ts`; `tsconfig.web.json` includes `src/preload/*.d.ts`
