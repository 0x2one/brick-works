...
- **main**: `src/main/index.ts` — frameless window (`frame: false`, `sandbox: false`) plus persistent services: LAN transfer (`lan-server.ts`), SSH tunnels (`ssh-manager.ts`/`ssh-store.ts`), SSH client shell/SFTP (`ssh-client-manager.ts`, shares `ssh-store`), K8s (`k8s-manager.ts`/`k8s-store.ts`)
...
- **routing**: HashRouter in `App.tsx` — `AppLayout` (Sider + `TitleBar`) with routes `/dev-tools`, `/dev-tools/:toolId`, `/memo-sticky`, `/lan-transfer`, `/ssh-tunnel`, `/ssh-client`, `/k8s`, `/about`; `/` redirects to `/dev-tools`. `AppLayout` syncs the LAN server language via `window.api.lan.setLang` on i18n change
...
- **IPC pattern**: adding a channel touches 3 places — `ipcMain.handle` in `src/main/index.ts` → preload bridge in `src/preload/index.ts` → types in `src/preload/index.d.ts`. Long-running services push state back via `webContents.send` (`lan:status-change`, `ssh:status-change`, `ssh:log`, `ssh:shell-data`, `ssh:shell-exit`, `k8s:status-change`, `k8s:log-chunk`, `k8s:exec-data`, `k8s:exec-exit`, `k8s:portforward-status`); K8s exec / SSH shell I/O goes over base64 (`k8s:writeExec` → `k8s:exec-data`, `ssh:writeShell` → `ssh:shell-data`), rendered in xterm
